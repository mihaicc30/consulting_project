...
<div :class="{ 'col-span-2': !isActive }" x-data="{ contactFilter: 'ana' }">
  ...
  <div class="flex flex-wrap justify-center py-2 px-2 transition-all">
    ...
    <!-- Contact Card - START -->
    <form data-email="{{$contact->email}}" data-name="{{$contact->name}}" data-contact class="flex flex-col max-w-[200px] shadow-xl p-2 m-2 rounded hover:scale-[1.1] transition-all duration-500" >
      @csrf
      ...
      <input type="text" name="name" value="{{ $contact->name }}" hidden>
      <p class="text-xs text-center line-clamp-1 font-bold" title="{{ $contact->name }}" name="name">
        {{ $contact->name }}
      </p>
